...
func GetPOI(poiID int) (result bool, poiList *black_kiwi_data_structs.PoiItem) {
	...
	if err != nil {
		...
		log.WithFields(log.Fields{"POI ID": poiID, "error": err}).Error("QueryRow failed while retrieving POI.")
		return false, nil
	}
	...
	log.WithFields(log.Fields{"POI": poiItem}).Debug("QueryRow succeeded while retrieving POI.")
