...
def var_dummies_data(n, radius):
    center = [11.343083, 44.494332]

    pp = []

    uni = generate_pos(n, center, radius, genProbStat.generateUniformNoise, None, True)
    poi = generate_pos(n, center, radius, poisson, None, True)
    ...
    for dummies in range(1, n+1):
        ...
        for i in range(0, n-dummies+1):
            ...
            tri_centroid.append(genProbStat.centroidCalculationAll(tri[i:i+dummies]))

        uni_pp = utils.privacy_preservation_arr(center, uni_centroid)
        poi_pp = utils.privacy_preservation_arr(center, poi_centroid)
        gau_pp = utils.privacy_preservation_arr(center, gau_centroid)
        tri_pp = utils.privacy_preservation_arr(center, tri_centroid)

        for i in range(len(uni_pp)):
            pp.append({"dummies": dummies, "value": uni_pp[i], "type": "uniform"})
            pp.append({"dummies": dummies, "value": poi_pp[i], "type": "poisson"})
            pp.append({"dummies": dummies, "value": gau_pp[i], "type": "gaussian"})
            pp.append({"dummies": dummies, "value": tri_pp[i], "type": "triangular"})
